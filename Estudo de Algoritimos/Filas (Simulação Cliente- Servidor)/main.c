...
void efetua_processamento(tipo_fila fila[MAX]){
   ...
   while(1) {
     limpar_tela();
     s = sorteia();
     if (s >=0 && s <=2) colocaNovoNaFila(&fila[s]);
     else if (s >=3 && s <=5) enviaParaProcessador(&fila[s-3], &fila[MAX-1]);
     else if (s >=6 && s <=10) retiraDoProcessador(&fila[MAX-1]);
     montar_tela(fila);
   }
}
